...
export async function startSimulateTrip(userID, scooterID) {
  try {
    ...
    if (!parkingSpot)
      return console.warn(
        "No parking spot available. Please try another location.",
      )
...
export async function simulateWithUsers() {
  try {
    ...
    if (scooters && scooters.length > 0) {
      ...
      const numRenting = Math.floor(
        (RENTING_PERCENTAGE / 100) * customers.length,
      )
      ...
      console.log(
        `All simulations completed (${simulationPromises.length} simulations).`,
      )
    } else {
