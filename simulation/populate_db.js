...
export const randomUser = {
  ...
  email: faker.internet
    .email(faker.person.firstName(), faker.person.lastName())
    .toLowerCase(),
...
export async function resetAllScooters() {
  try {
    ...
    console.log(
      `${updatedScooters.length} scooters have been reset to default values.`,
    )
  } catch (error) {
...
export async function addMultipleScooters(number, city) {
  for (let i = 0; i < number; i++) {
    ...
    console.log(
      `Added scooter ${i + 1}: Location [${newScooter.location.latitude}, ${newScooter.location.longitude}]`,
    )
  }
...
export async function addMultipleScootersRotatingCities(number) {
  ...
  for (let i = 0; i < number; i++) {
    ...
    await addScooter(newScooter)
    console.log(
      `Added scooter ${i + 1} in ${city}: Location [${newScooter.location.latitude}, ${newScooter.location.longitude}]`,
    )
  }
